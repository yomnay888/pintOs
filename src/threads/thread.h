...
struct thread
{
   ...
   int og_priority; /* Original priority. */
   struct list locks_granted;
   struct lock *lock_blocked_for; // lock the thread blocked by
   bool is_donated;
...
void modify_thread_priority(struct thread *);
...
bool less(struct list_elem *elem1, struct list_elem *elem2, void *aux);
